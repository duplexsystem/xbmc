...
bool CDVDVideoCodecFFmpeg::GetPictureCommon(VideoPicture* pVideoPicture)
{
  ...
  if (pVideoPicture->hdrType == StreamHdrType::HDR_TYPE_DOLBYVISION)
  {
    ...
    if (sd)
    {
      ...
      // Incoming branch's Dolby Vision specific logic
      if (header != nullptr && header->el_spatial_resampling_filter_flag == 1 &&
      ...
      // HEAD branch's libplacebo mapping for Dolby Vision metadata
  ...
  // Incoming branch's HDR10+ detection

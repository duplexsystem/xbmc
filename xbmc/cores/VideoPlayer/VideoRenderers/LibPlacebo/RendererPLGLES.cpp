                                ...
                                unsigned int orientation)
{
  ...
  if (!m_plQueue)
  {
    ...
    if (!m_plQueue)
      CLog::Log(LOGERROR, "CRendererPLGLES::Configure - pl_queue_create failed");
  }
...
bool CRendererPLGLES::Flush(bool saveBuffers)
{
  if (m_plQueue)
  {
  ...
  return CLinuxRendererGLES::Flush(saveBuffers);
}

void CRendererPLGLES::AddVideoPicture(const VideoPicture& picture, int index)
{
  ...
  src.discard = &CRendererPLGLES::DiscardCallback;
...
void CRendererPLGLES::DiscardCallback(const struct pl_source_frame* src)
{
  delete static_cast<QueuedFrameState*>(src->frame_data);
}
...
bool CRendererPLGLES::UploadTexture(int index)
{
  ...
  if (drmBuf)
  {
        ...
        buf.plColorRepr.dovi != nullptr)
    {
      if (buf.plColorRepr.dovi != nullptr)
      {
        // Dolby Vision: pl_map_avdovi_metadata fills transfer, primaries, and hdr
        plbuf.colorSpace = buf.plColorSpace;
      }
      else
      {
        // HDR10 / HDR10+: pl_map_hdr_metadata only populates .hdr; merge to
        // preserve pl_transfer_from_av / pl_primaries_from_av set above.
        pl_hdr_metadata_merge(&plbuf.colorSpace.hdr, &buf.plColorSpace.hdr);
      }
      plbuf.colorRepr = buf.plColorRepr;
      plbuf.doviMetadata = buf.plDoviMetadata;
...
bool CRendererPLGLES::RenderHook(int idx)
{
  ...
  // m_passthroughHDR is set by CLinuxRendererGLES::Configure when SetHDR() succeeds,
  ...
  if (m_plQueue && m_queuePtsOffsetSet)
  {
    ...
    if (status == PL_QUEUE_OK || status == PL_QUEUE_MORE)
    {
      if (!pl_render_image_mix(renderer, &mix, &frameOut, &params))
        CLog::Log(LOGWARNING, "CRendererPLGLES::RenderHook - pl_render_image_mix failed");
      rendered = true;
    }
    else if (status == PL_QUEUE_ERR)
    {
      CLog::Log(LOGWARNING, "CRendererPLGLES::RenderHook - pl_queue_update error");
    }
  }
  if (!rendered)
  {
    ...
    if (!pl_render_image(renderer, &frameIn, &frameOut, &params))
      CLog::Log(LOGWARNING, "CRendererPLGLES::RenderHook - pl_render_image failed");
  }

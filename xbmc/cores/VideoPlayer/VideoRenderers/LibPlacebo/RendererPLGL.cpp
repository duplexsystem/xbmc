...
CBaseRenderer* CRendererPLGL::Create(CVideoBuffer* buffer)
{
  ...
  // Accept SW-decodable formats, VAAPI surfaces, and DRMPRIME DMA-buf buffers.
  // Any other hardware-specific buffer type is rejected so the factory falls
  // back to the platform-native renderer.
  ...
  {
    CLog::Log(LOGDEBUG,
              "CRendererPLGL::Create - unsupported buffer type (format {}), skipping",
...
bool CRendererPLGL::Configure(const VideoPicture& picture, float fps, unsigned int orientation)
{
  if (!CLinuxRendererGL::Configure(picture, fps, orientation))
    return false;
...
bool CRendererPLGL::ConfigChanged(const VideoPicture& picture)
{
...
bool CRendererPLGL::Supports(ERENDERFEATURE feature) const
{
  switch (feature)
  {
...
bool CRendererPLGL::Supports(ESCALINGMETHOD method) const
{
  // libplacebo has its own high-quality scaling filter chain—all methods map to it
  switch (method)
  {
...
void CRendererPLGL::UpdateVideoFilter()
{
...
EShaderFormat CRendererPLGL::GetShaderFormat()
{
  // We bypass the shader entirely via RenderHook; return NONE to prevent
  // CLinuxRendererGL from loading its own YUV shaders for our buffers.
  return SHADER_NONE;
}

// ---------------------------------------------------------------------------
// Shader hook — tell LinuxRendererGL we handle rendering ourselves
// ---------------------------------------------------------------------------

bool CRendererPLGL::LoadShadersHook()
{
  // Returning true prevents CLinuxRendererGL::LoadShaders from loading its
  // own YUV shaders. We set m_renderMethod to a sentinel so ValidateRenderer
  // doesn't reject us.
  m_renderMethod = RENDER_GLSL; // reuse value; actual render is in RenderHook
  return true;
}

// ---------------------------------------------------------------------------
// Texture lifecycle — we manage pl_tex ourselves, not GL textures
// ---------------------------------------------------------------------------
...
bool CRendererPLGL::UploadTexture(int index)
{
  CPictureBuffer& buf = m_buffers[index];
  if (!buf.videoBuffer)
    return false;
  ...
  //
  // We create EGLImages manually (like CVaapi2Texture) and bind them to GL_TEXTURE_2D
  // via GL_OES_EGL_image. This avoids two problems with pl_tex_create(PL_HANDLE_DMA_BUF):
  //  1. libplacebo always passes the DRM modifier to eglCreateImageKHR, which some Intel
  //     drivers reject for formats like DRM_FORMAT_GR88 even when the modifier is valid.
  //  2. GL_TEXTURE_EXTERNAL_OES cannot be sampled in desktop GL Core Profile GLSL shaders.
  if (m_isVAAPI)
  {
    ...
    {
      CLog::Log(LOGERROR, "CRendererPLGL::UploadTexture - no valid VAAPI surface");
      return false;
    }
    ...
    {
      CLog::Log(LOGERROR,
                "CRendererPLGL::UploadTexture - EGL interop functions not available for VAAPI");
      return false;
    }
    ...
    {
      CLog::Log(LOGERROR, "CRendererPLGL::UploadTexture - vaExportSurfaceHandle failed: {}",
                vaErrorStr(vaStatus));
      return false;
    }
    ...
    for (uint32_t i = 0; i < numLayers && success; ++i)
    {
      ...
      // Build EGL attribute list for DMA-buf import (max 6 static + 2 modifier = 8 pairs + EGL_NONE)
      EGLint attribs[17];
      EGLint* a = attribs;
      ...
      *a++ = EGL_NONE;
      ...
      {
        CLog::Log(LOGERROR,
                  "CRendererPLGL::UploadTexture - eglCreateImageKHR failed for VAAPI plane {} "
      ...
      // Bind EGLImage to GL_TEXTURE_2D via GL_OES_EGL_image (available on Mesa desktop GL).
      // GL_TEXTURE_EXTERNAL_OES is NOT used here because samplerExternalOES is unsupported
      // in GLSL on desktop GL 4.6 Core Profile (Mesa Intel). GL_TEXTURE_2D works correctly.
      ...
      switch (layer.drm_format)
      {
        ...
        default:
          CLog::Log(LOGERROR,
                    "CRendererPLGL::UploadTexture - unsupported DRM fourcc 0x{:x} for VAAPI plane {}",
                    layer.drm_format, i);
          success = false;
          break;
      }
      ...
      {
        CLog::Log(LOGERROR,
                  "CRendererPLGL::UploadTexture - pl_opengl_wrap failed for VAAPI plane {}", i);
        success = false;
      }
    }
    ...
    if (!success)
    {
      // ReleasePLBuffer cleans up any partial state (GL textures, EGLImages, fds)
      ReleasePLBuffer(index);
      return false;
    }
  ...
  if (m_isDRMPRIME)
  {
    ...
    {
      CLog::Log(LOGERROR, "CRendererPLGL::UploadTexture - CDRMPRIMETexture::Map failed");
      return false;
    }
    ...
    if (!plbuf.tex[0])
    {
      CLog::Log(LOGERROR, "CRendererPLGL::UploadTexture - pl_opengl_wrap failed for DRMPRIME");
    ...
    // EGL_EXT_image_dma_buf_import + GL_TEXTURE_EXTERNAL_OES: the GPU driver
    // applies the YCbCr→RGB matrix when the texture is sampled, yielding RGB
    // in the source primaries/transfer. Tell libplacebo this is RGB so it
    // only performs tone-mapping, not a redundant matrix conversion.
  ...
  {
    CLog::Log(LOGERROR, "CRendererPLGL::UploadTexture - unsupported pixel format {} (buf reports {})",
  ...
  // --- HDR static metadata (mastering display + CLL) ---
  if (buf.m_srcColTransfer == AVCOL_TRC_SMPTEST2084 ||
      buf.m_srcColTransfer == AVCOL_TRC_ARIB_STD_B67)
  {
    pl_hdr_metadata& hdr = plbuf.colorSpace.hdr;
...
bool CRendererPLGL::RenderHook(int idx)
{
  ...
  // --- Rotation: map Kodi degrees (CCW) to libplacebo pl_rotation ---
  // Kodi stores m_renderOrientation in degrees counter-clockwise.
  // pl_rotation is CW (0/90/180/270). So CCW 90 == CW 270, etc.
  switch (m_renderOrientation)
  {
    case 90:  frameIn.rotation = PL_ROTATION_270; break; // CCW 90 -> CW 270
    case 180: frameIn.rotation = PL_ROTATION_180; break;
    case 270: frameIn.rotation = PL_ROTATION_90;  break; // CCW 270 -> CW 90
  ...
  if (!outTex)
  {
    CLog::Log(LOGERROR, "CRendererPLGL::RenderHook - failed to wrap GL framebuffer");
    return false;
  }
  ...
  // Derive final render params from pl_options; apply Kodi-specific override
  ...
  // Kodi's GL renderer creates m_vertexArray at startup and keeps it bound.
  // libplacebo binds its own VAO per-pass (gpu_pass.c:616) and resets to VAO 0
  // at line 675. In GL core profile, VAO 0 is invalid: glVertexAttribPointer and
  // glDrawElements both generate GL_INVALID_OPERATION, making the GUI invisible.
  GLint savedVAO{};
  ...
  // Restore Kodi's VAO: libplacebo resets to VAO 0 at gpu_pass.c:675. In GL core
  // profile VAO 0 is not a valid draw target; glVertexAttribPointer and glDrawElements
  // both generate GL_INVALID_OPERATION, making the GUI invisible.
  glBindVertexArray(savedVAO);

  // Reset active texture unit: libplacebo binds textures to multiple units and may
  // leave the active unit set to something other than GL_TEXTURE0. Kodi's GUI texture
  // code calls glActiveTexture itself (CGLTexture::BindToUnit), but resetting here
  // avoids any unit-mismatch bugs if a code path skips that call.
  ...
  // Destroy the libplacebo wrapper — does NOT delete the actual GL framebuffer
  pl_tex_destroy(gpu, &outTex);

  // Always return true: we own this buffer slot and the base class must not
  // fall through to RenderSinglePass (which would crash on our uninitialized GL planes).
  return true;
}
...
void CRendererPLGL::ReleasePLBuffer(int index)
{
  PLBuffer& plbuf = m_plBuffers[index];
  ...
  if (m_isVAAPI)
  {
    ...
    {
      // pl_tex_destroy on a wrapped texture only destroys the pl wrapper, not the GL texture
      if (plbuf.tex[n])
      {
